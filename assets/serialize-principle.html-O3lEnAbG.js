...
</code></pre><h2 id="序列化及反序列化相关知识" tabindex="-1"><a class="header-anchor" href="#序列化及反序列化相关知识" aria-hidden="true">#</a> 序列化及反序列化相关知识</h2><p>1、在Java中，只要一个类实现了<code>java.io.Serializable</code>接口，那么它就可以被序列化。</p><p>2、通过<code>ObjectOutputStream</code>和<code>ObjectInputStream</code>对对象进行序列化及反序列化</p><p>3、虚拟机是否允许反序列化，不仅取决于类路径和功能代码是否一致，一个非常重要的一点是两个类的序列化 ID 是否一致（就是 <code>private static final long serialVersionUID</code>）</p><p>4、序列化并不保存静态变量。</p><p>5、要想将父类对象也序列化，就需要让父类也实现<code>Serializable</code> 接口。</p><p>6、Transient 关键字的作用是控制变量的序列化，在变量声明前加上该关键字，可以阻止该变量被序列化到文件中，在被反序列化后，transient 变量的值被设为初始值，如 int 型的是 0，对象型的是 null。</p><p>7、服务器端给客户端发送序列化对象数据，对象中有一些数据是敏感的，比如密码字符串等，希望对该密码字段在序列化时，进行加密，而客户端如果拥有解密的密钥，只有在客户端进行反序列化时，才可以对密码进行读取，这样可以一定程度保证序列化对象的数据安全。</p><h2 id="arraylist的序列化" tabindex="-1"><a class="header-anchor" href="#arraylist的序列化" aria-hidden="true">#</a> ArrayList的序列化</h2><p>在介绍ArrayList序列化之前，先来考虑一个问题：</p><blockquote><p><strong>如何自定义的序列化和反序列化策略</strong></p></blockquote><p>带着这个问题，我们来看<code>java.util.ArrayList</code>的源码</p><p>code 3</p><pre><code>public class ArrayList&lt;E&gt; extends AbstractList&lt;E&gt;
...
</code></pre><p>了解ArrayList的人都知道，ArrayList底层是通过数组实现的。那么数组<code>elementData</code>其实就是用来保存列表中的元素的。通过该属性的声明方式我们知道，他是无法通过序列化持久化下来的。那么为什么code 4的结果却通过序列化和反序列化把List中的元素保留下来了呢？</p><h3 id="writeobject和readobject方法" tabindex="-1"><a class="header-anchor" href="#writeobject和readobject方法" aria-hidden="true">#</a> writeObject和readObject方法</h3><p>在ArrayList中定义了来个方法： <code>writeObject</code>和<code>readObject</code>。</p><p>这里先给出结论:</p><blockquote><p>在序列化过程中，如果被序列化的类中定义了writeObject 和 readObject 方法，虚拟机会试图调用对象类里的 writeObject 和 readObject 方法，进行用户自定义的序列化和反序列化。</p><p>如果没有这样的方法，则默认调用是 ObjectOutputStream 的 defaultWriteObject 方法以及 ObjectInputStream 的 defaultReadObject 方法。</p><p>用户自定义的 writeObject 和 readObject 方法可以允许用户控制序列化的过程，比如可以在序列化的过程中动态改变序列化的数值。</p></blockquote><p>来看一下这两个方法的具体实现：</p><p>code 5</p><pre><code>private void readObject(java.io.ObjectInputStream s)
...
</code></pre><p>那么为什么ArrayList要用这种方式来实现序列化呢？</p><h3 id="why-transient" tabindex="-1"><a class="header-anchor" href="#why-transient" aria-hidden="true">#</a> why transient</h3><p>ArrayList实际上是动态数组，每次在放满以后自动增长设定的长度值，如果数组自动增长长度设为100，而实际只放了一个元素，那就会序列化99个null元素。为了保证在序列化的时候不会将这么多null同时进行序列化，ArrayList把元素数组设置为transient。</p><h3 id="why-writeobject-and-readobject" tabindex="-1"><a class="header-anchor" href="#why-writeobject-and-readobject" aria-hidden="true">#</a> why writeObject and readObject</h3><p>前面说过，为了防止一个包含大量空对象的数组被序列化，为了优化存储，所以，ArrayList使用<code>transient</code>来声明<code>elementData</code>。 但是，作为一个集合，在序列化过程中还必须保证其中的元素可以被持久化下来，所以，通过重写<code>writeObject</code> 和 <code>readObject</code>方法的方式把其中的元素保留下来。</p><p><code>writeObject</code>方法把<code>elementData</code>数组中的元素遍历的保存到输出流（ObjectOutputStream）中。</p><p><code>readObject</code>方法从输入流（ObjectInputStream）中读出对象并保存赋值到<code>elementData</code>数组中。</p><p>至此，我们先试着来回答刚刚提出的问题：</p><blockquote><p>如何自定义的序列化和反序列化策略</p></blockquote><p>答：可以通过在被序列化的类中增加writeObject 和 readObject方法。那么问题又来了：</p><blockquote><p>虽然ArrayList中写了writeObject 和 readObject 方法，但是这两个方法并没有显示的被调用啊。</p><p><strong>那么如果一个类中包含writeObject 和 readObject 方法，那么这两个方法是怎么被调用的呢?</strong></p></blockquote><h2 id="objectoutputstream" tabindex="-1"><a class="header-anchor" href="#objectoutputstream" aria-hidden="true">#</a> ObjectOutputStream</h2><p>从code 4中，我们可以看出，对象的序列化过程通过ObjectOutputStream和ObjectInputputStream来实现的，那么带着刚刚的问题，我们来分析一下ArrayList中的writeObject 和 readObject 方法到底是如何被调用的呢？</p><p>为了节省篇幅，这里给出ObjectOutputStream的writeObject的调用栈：</p><p><code>writeObject ---&gt; writeObject0 ---&gt;writeOrdinaryObject---&gt;writeSerialData---&gt;invokeWriteObject</code></p><p>这里看一下invokeWriteObject：</p><pre><code>void invokeWriteObject(Object obj, ObjectOutputStream out)
...
</code></pre><p>在进行序列化操作时，会判断要被序列化的类是否是Enum、Array和Serializable类型，如果不是则直接抛出<code>NotSerializableException</code>。</p><h2 id="总结" tabindex="-1"><a class="header-anchor" href="#总结" aria-hidden="true">#</a> 总结</h2><p>1、如果一个类想被序列化，需要实现Serializable接口。否则将抛出<code>NotSerializableException</code>异常，这是因为，在序列化操作过程中会对类型进行检查，要求被序列化的类必须属于Enum、Array和Serializable类型其中的任何一种。</p><p>2、在变量声明前加上该关键字，可以阻止该变量被序列化到文件中。</p><p>3、在类中增加writeObject 和 readObject 方法可以实现自定义序列化策略</p><h2 id="参考资料" tabindex="-1"><a class="header-anchor" href="#参考资料" aria-hidden="true">#</a> 参考资料</h2>`,79),d={href:"https://www.ibm.com/developerworks/cn/java/j-lo-serial/",target:"_blank",rel:"noopener noreferrer"};function u(b,h){const t=a("ExternalLinkIcon");return i(),r("div",null,[l,e("p",null,[e("a",d,[o("Java 序列化的高级认识"),c(t)])])])}const O=n(s,[["render",u],["__file","serialize-principle.html.vue"]]);export{O as default};
