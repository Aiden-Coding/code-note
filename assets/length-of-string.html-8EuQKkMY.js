import{_ as r,r as i,o,c as p,a as e,b as t,d as a,e as c}from"./app-3RcBQnkC.js";const s={},d=e("p",null,"关于String有没有长度限制的问题，我之前单独写过一篇文章分析过，最近我又抽空回顾了一下这个问题，发现又有了一些新的认识。于是准备重新整理下这个内容。",-1),h=e("p",null,"这次在之前那篇文章的基础上除了增加了一些验证过程外，还有些错误内容的修正。我这次在分析过程中会尝试对Jdk的编译过程进行debug，并且会参考一些JVM规范等全方面的介绍下这个知识点。",-1),l={href:"https://www.bilibili.com/video/BV1uK4y1t7H1/",target:"_blank",rel:"noopener noreferrer"},g=c(`<h3 id="string的长度限制" tabindex="-1"><a class="header-anchor" href="#string的长度限制" aria-hidden="true">#</a> String的长度限制</h3><p>想要搞清楚这个问题，首先我们需要翻阅一下String的源码，看下其中是否有关于长度的限制或者定义。</p><p>String类中有很多重载的构造函数，其中有几个是支持用户传入length来执行长度的：</p><pre><code>public String(byte bytes[], int offset, int length) 
...
</code></pre><p>那么，明明String的构造函数指定的长度是可以支持2147483647(2^31 - 1)的，为什么像以上形式定义的时候无法编译呢？</p><p>其实，形如<code>String s = &quot;xxx&quot;;</code>定义String的时候，xxx被我们称之为字面量，这种字面量在编译之后会以常量的形式进入到Class常量池。</p><p>那么问题就来了，因为要进入常量池，就要遵守常量池的有关规定。</p><h3 id="常量池限制" tabindex="-1"><a class="header-anchor" href="#常量池限制" aria-hidden="true">#</a> 常量池限制</h3><p>我们知道，javac是将Java文件编译成class文件的一个命令，那么在Class文件生成过程中，就需要遵守一定的格式。</p><p>根据《Java虚拟机规范》中第4.4章节常量池的定义，CONSTANT_String_info 用于表示 java.lang.String 类型的常量对象，格式如下：</p><pre><code>CONSTANT_String_info {
    u1 tag;
    u2 string_index;
}
...
</code></pre><p>得到的字符串长度就有10万，另外我之前在实际应用中遇到过这个问题。</p><p>之前一次系统对接，需要传输高清图片，约定的传输方式是对方将图片转成BASE64编码，我们接收到之后再转成图片。</p><p>在将BASE64编码后的内容赋值给字符串的时候就抛了异常。</p><h3 id="总结" tabindex="-1"><a class="header-anchor" href="#总结" aria-hidden="true">#</a> 总结</h3><p>字符串有长度限制，在编译期，要求字符串常量池中的常量不能超过65535，并且在javac执行过程中控制了最大值为65534。</p><p>在运行期，长度不能超过Int的范围，否则会抛异常。</p>`,50),u={href:"https://www.bilibili.com/video/BV1uK4y1t7H1/",target:"_blank",rel:"noopener noreferrer"};function _(S,f){const n=i("ExternalLinkIcon");return o(),p("div",null,[d,h,e("p",null,[t("因为这个问题涉及到Java的编译原理相关的知识，所以通过视频的方式讲解会更加容易理解一些，视频我上传到了B站："),e("a",l,[t("【灵魂拷问】Java中的String到底有没有长度限制？"),a(n)])]),g,e("p",null,[t("最后，这个知识点 ，我录制了视频("),e("a",u,[t("点击跳转"),a(n)]),t(") ，其中有关于如何进行实验测试、如何查阅Java规范以及如何对javac进行deubg的技巧。欢迎进一步学习。")])])}const v=r(s,[["render",_],["__file","length-of-string.html.vue"]]);export{v as default};
