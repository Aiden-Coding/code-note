import{_ as r,r as i,o as p,c as m,a as t,b as e,d as n,e as o}from"./app-3RcBQnkC.js";const l={},d=o(`<p>在日常开发中，我们经常会用到时间，我们有很多办法在Java代码中获取时间。但是不同的方法获取到的时间的格式都不尽相同，这时候就需要一种格式化工具，把时间显示成我们需要的格式。</p><p>最常用的方法就是使用SimpleDateFormat类。这是一个看上去功能比较简单的类，但是，一旦使用不当也有可能导致很大的问题。</p><p>在阿里巴巴Java开发手册中，有如下明确规定：</p><img src="https://www.hollischuang.com/wp-content/uploads/2018/11/规约1.png" alt="" width="1862" height="154" class="aligncenter size-full wp-image-3043"><p>那么，本文就围绕SimpleDateFormat的用法、原理等来深入分析下如何以正确的姿势使用它。</p><h3 id="simpledateformat用法" tabindex="-1"><a class="header-anchor" href="#simpledateformat用法" aria-hidden="true">#</a> SimpleDateFormat用法</h3><p>SimpleDateFormat是Java提供的一个格式化和解析日期的工具类。它允许进行格式化（日期 -&gt; 文本）、解析（文本 -&gt; 日期）和规范化。SimpleDateFormat 使得可以选择任何用户定义的日期-时间格式的模式。</p><p>在Java中，可以使用SimpleDateFormat的format方法，将一个Date类型转化成String类型，并且可以指定输出格式。</p><pre><code>// Date转String
Date data = new Date();
...
</code></pre><p><strong>这种定义方式，存在很大的安全隐患。</strong></p><h4 id="问题重现" tabindex="-1"><a class="header-anchor" href="#问题重现" aria-hidden="true">#</a> 问题重现</h4><p>我们来看一段代码，以下代码使用线程池来执行时间输出。</p><pre><code>   /** * @author Hollis */ 
   public class Main {
...
</code></pre><h3 id="总结" tabindex="-1"><a class="header-anchor" href="#总结" aria-hidden="true">#</a> 总结</h3><p>本文介绍了SimpleDateFormat的用法，SimpleDateFormat主要可以在String和Date之间做转换，还可以将时间转换成不同时区输出。同时提到在并发场景中SimpleDateFormat是不能保证线程安全的，需要开发者自己来保证其安全性。</p><p>主要的几个手段有改为局部变量、使用synchronized加锁、使用Threadlocal为每一个线程单独创建一个等。</p><p>希望通过此文，你可以在使用SimpleDateFormat的时候更加得心应手。</p>`,39);function u(D,S){const a=i("ExternalLinkIcon");return p(),m("div",null,[d,t("blockquote",null,[t("p",null,[e("上面的例子我特意写的稍微复杂了一些，不过我几乎都加了注释。这里面涉及到了"),t("a",c,[e("线程池的创建"),n(a)]),e("、"),t("a",s,[e("CountDownLatch"),n(a)]),e("、lambda表达式、线程安全的HashSet等知识。感兴趣的朋友可以逐一了解一下。")])]),h])}const F=r(l,[["render",u],["__file","simpledateformat-thread-safe.html.vue"]]);export{F as default};
