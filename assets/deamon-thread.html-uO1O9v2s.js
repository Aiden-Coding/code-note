import{_ as e,o as n,c as t,e as a}from"./app-cCF93fuz.js";const r={},o=a(`<p>在Java中有两类线程：User Thread(用户线程)、Daemon Thread(守护线程) 。用户线程一般用户执行用户级任务，而守护线程也就是“后台线程”，一般用来执行后台任务，守护线程最典型的应用就是GC(垃圾回收器)。</p><p>这两种线程其实是没有什么区别的，唯一的区别就是Java虚拟机在所有“用户线程”都结束后就会退出。</p><p>我们可以通过使用<code>setDaemon()</code>方法通过传递true作为参数，使线程成为一个守护线程。我们必须在启动线程之前调用一个线程的<code>setDaemon()</code>方法。否则，就会抛出一个<code>java.lang.IllegalThreadStateException</code>。</p><p>可以使用<code>isDaemon()</code>方法来检查线程是否是守护线程。</p><pre><code>/**
 * @author Hollis
 */
public class Main {
