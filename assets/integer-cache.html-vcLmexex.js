...
</code></pre><p>在创建对象之前先从IntegerCache.cache中寻找。如果没找到才使用new新建对象。</p><h2 id="integercache-class" tabindex="-1"><a class="header-anchor" href="#integercache-class" aria-hidden="true">#</a> IntegerCache Class</h2><p>IntegerCache是Integer类中定义的一个<code>private static</code>的内部类。接下来看看他的定义。</p><pre><code>  /**
...
</code></pre><p>其中的javadoc详细的说明了缓存支持-128到127之间的自动装箱过程。最大值127可以通过<code>-XX:AutoBoxCacheMax=size</code>修改。 缓存通过一个for循环实现。从低到高并创建尽可能多的整数并存储在一个整数数组中。这个缓存会在Integer类第一次被使用的时候被初始化出来。以后，就可以使用缓存中包含的实例对象，而不是创建一个新的实例(在自动装箱的情况下)。</p><p>实际上这个功能在Java 5中引入的时候,范围是固定的-128 至 +127。后来在Java 6中，可以通过<code>java.lang.Integer.IntegerCache.high</code>设置最大值。这使我们可以根据应用程序的实际情况灵活地调整来提高性能。到底是什么原因选择这个-128到127范围呢？因为这个范围的数字是最被广泛使用的。 在程序中，第一次使用Integer的时候也需要一定的额外时间来初始化这个缓存。</p><h2 id="java语言规范中的缓存行为" tabindex="-1"><a class="header-anchor" href="#java语言规范中的缓存行为" aria-hidden="true">#</a> Java语言规范中的缓存行为</h2>`,19),f={href:"http://docs.oracle.com/javase/specs/jls/se8/html/jls-5.html#jls-5.1.7",target:"_blank",rel:"noopener noreferrer"},v=r('<blockquote><p>如果一个变量p的值是：</p><p>-128至127之间的整数(§3.10.1)</p><p>true 和 false的布尔值 (§3.10.3)</p><p>‘\\u0000’至 ‘\\u007f’之间的字符(§3.10.4)</p><p>中时，将p包装成a和b两个对象时，可以直接使用a==b判断a和b的值是否相等。</p></blockquote><h2 id="其他缓存的对象" tabindex="-1"><a class="header-anchor" href="#其他缓存的对象" aria-hidden="true">#</a> 其他缓存的对象</h2><p>这种缓存行为不仅适用于Integer对象。我们针对所有的整数类型的类都有类似的缓存机制。</p><blockquote><p>有ByteCache用于缓存Byte对象</p><p>有ShortCache用于缓存Short对象</p><p>有LongCache用于缓存Long对象</p><p>有CharacterCache用于缓存Character对象</p></blockquote><p><code>Byte</code>, <code>Short</code>, <code>Long</code>有固定范围: -128 到 127。对于<code>Character</code>, 范围是 0 到 127。除了<code>Integer</code>以外，这个范围都不能改变。</p>',5);function I(m,b){const t=i("ExternalLinkIcon");return c(),h("div",null,[n("p",null,[e("英文原文："),n("a",p,[e("Java Integer Cache"),a(t)]),e(" 翻译地址："),n("a",g,[e("Java中整型的缓存机制"),a(t)]),e(" 原文作者："),n("a",l,[e("Java Papers"),a(t)]),e(" 翻译作者："),n("a",d,[e("Hollis"),a(t)]),e(" 转载请注明出处。")]),u,n("p",null,[e("在"),n("a",f,[e("Boxing Conversion"),a(t)]),e("部分的Java语言规范(JLS)规定如下：")]),v])}const C=o(s,[["render",I],["__file","integer-cache.html.vue"]]);export{C as default};
