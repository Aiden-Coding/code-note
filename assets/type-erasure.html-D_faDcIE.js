...
</code></pre><p>第2个泛型类<code>Comparable &lt;A&gt;</code>擦除后 A被替换为最左边界<code>Object</code>。<code>Comparable&lt;NumericValue&gt;</code>的类型参数<code>NumericValue</code>被擦除掉，但是这直 接导致<code>NumericValue</code>没有实现接口<code>Comparable的compareTo(Object that)</code>方法，于是编译器充当好人，添加了一个<strong>桥接方法</strong>。 第3个示例中限定了类型参数的边界<code>&lt;A extends Comparable&lt;A&gt;&gt;A</code>，A必须为<code>Comparable&lt;A&gt;</code>的子类，按照类型擦除的过程，先讲所有的类型参数 ti换为最左边界<code>Comparable&lt;A&gt;</code>，然后去掉参数类型<code>A</code>，得到最终的擦除后结果。</p><hr><h3 id="四、泛型带来的问题" tabindex="-1"><a class="header-anchor" href="#四、泛型带来的问题" aria-hidden="true">#</a> 四、泛型带来的问题</h3><p><strong>一、当泛型遇到重载：</strong></p><pre><code>public class GenericTypes {  
...
</code></pre>`,22),x=t("code",null,"List<String>",-1),_=t("code",null,"List<Integer>",-1),C=t("code",null,"List<Integer>",-1),S=t("code",null,"List<String>",-1),q=t("p",null,[t("strong",null,"二、当泛型遇到catch:")],-1),y=c(`<p>三、当泛型内包含静态变量</p><pre><code>public class StaticTest{
...
</code></pre><p>答案是——2！由于经过类型擦除，所有的泛型类实例都关联到同一份字节码上，泛型类的所有静态变量是共享的。</p><hr><h3 id="五、总结" tabindex="-1"><a class="header-anchor" href="#五、总结" aria-hidden="true">#</a> 五、总结</h3><p>1.虚拟机中没有泛型，只有普通类和普通方法,所有泛型类的类型参数在编译时都会被擦除,泛型类并没有自己独有的Class类对象。比如并不存在<code>List&lt;String&gt;</code>.class或是<code>List&lt;Integer&gt;.class</code>，而只有<code>List.class</code>。 2.创建泛型对象时请指明类型，让编译器尽早的做参数检查（<strong>Effective Java，第23条：请不要在新代码中使用原生态类型</strong>） 3.不要忽略编译器的警告信息，那意味着潜在的<code>ClassCastException</code>等着你。 4.静态变量是被泛型类的所有实例所共享的。对于声明为<code>MyClass&lt;T&gt;</code>的类，访问其中的静态变量的方法仍然是 <code>MyClass.myStaticVar</code>。不管是通过<code>new MyClass&lt;String&gt;</code>还是<code>new MyClass&lt;Integer&gt;</code>创建的对象，都是共享一个静态变量。 5.泛型的类型参数不能用在<code>Java</code>异常处理的<code>catch</code>语句中。因为异常处理是由JVM在运行时刻来进行的。由于类型信息被擦除，<code>JVM</code>是无法区分两个异常类型<code>MyException&lt;String&gt;</code>和<code>MyException&lt;Integer&gt;</code>的。对于<code>JVM</code>来说，它们都是 <code>MyException</code>类型的。也就无法执行与异常对应的<code>catch</code>语句。</p>`,6);function f(L,I){const r=o("ExternalLinkIcon"),i=o("e"),l=o("Integer"),s=o("String"),d=o("t");return u(),g("div",null,[h,t("blockquote",null,[t("p",null,[e("类型擦除指的是通过类型参数合并，将泛型类型实例关联到同一份字节码上。编译器只为泛型类型生成一份字节码，并将其实例关联到这份字节码上。类型擦除的关键在于从泛型类型中清除类型参数的相关信息，并且再必要的时候添加类型检查和类型转换的方法。 类型擦除可以简单的理解为将泛型java代码转换为普通java代码，只不过编译器更直接点，将泛型java代码直接转换成普通java字节码。 类型擦除的主要过程如下： 1.将所有的泛型参数用其最左边界（最顶级的父类型）类型替换。（这部分内容可以看："),t("a",b,[e("Java泛型中extends和super的理解"),n(r)]),e("） 2.移除所有的类型参数。")])]),v,t("p",null,[e("上面这段代码，有两个重载的函数，因为他们的参数类型不同，一个是"),x,e("另一个是"),_,e(" ，但是，这段代码是编译通不过的。因为我们前面讲过，参数"),C,e("和"),S,e("编译之后都被擦除了，变成了一样的原生类型List"),n(i,null,{default:a(()=>[e("，擦除动作导致这两个方法的特征签名变得一模一样。")]),_:1})]),q,t("p",null,[e("如果我们自定义了一个泛型异常类GenericException"),n(d,null,{default:a(()=>[e("，那么，不要尝试用多个catch取匹配不同的异常类型，例如你想要分别捕获GenericException"),n(s,null,{default:a(()=>[e("、GenericException"),n(l,null,{default:a(()=>[e("，这也是有问题的。")]),_:1})]),_:1})]),_:1})]),y])}const V=p(m,[["render",f],["__file","type-erasure.html.vue"]]);export{V as default};
