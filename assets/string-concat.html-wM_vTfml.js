...
</code></pre><p>并且，Java8中的String类中也提供了一个静态的join方法，用法和StringUtils.join类似。</p><p>以上就是比较常用的五种在Java种拼接字符串的方式，那么到底哪种更好用呢？为什么阿里巴巴Java开发手册中不建议在循环体中使用<code>+</code>进行字符串拼接呢？</p><p><img src="https://www.hollischuang.com/wp-content/uploads/2019/01/15472850170230.jpg" alt="" style="width:917px;">￼</p><p>(阿里巴巴Java开发手册中关于字符串拼接的规约)</p><h3 id="使用-拼接字符串的实现原理" tabindex="-1"><a class="header-anchor" href="#使用-拼接字符串的实现原理" aria-hidden="true">#</a> 使用<code>+</code>拼接字符串的实现原理</h3><p>关于这个知识点，前面的章节介绍过，主要是通过StringBuilder的append方法实现的。</p><h3 id="concat是如何实现的" tabindex="-1"><a class="header-anchor" href="#concat是如何实现的" aria-hidden="true">#</a> concat是如何实现的</h3><p>我们再来看一下concat方法的源代码，看一下这个方法又是如何实现的。</p><pre><code>public String concat(String str) {
...
</code></pre><h3 id="效率比较" tabindex="-1"><a class="header-anchor" href="#效率比较" aria-hidden="true">#</a> 效率比较</h3><p>既然有这么多种字符串拼接的方法，那么到底哪一种效率最高呢？我们来简单对比一下。</p><pre><code>long t1 = System.currentTimeMillis();
//这里是初始字符串定义
